...
def calculate_ROI2(place_x, place_x_df, gdf_full_updated,
                   total_investment_place_x,
                   predicted_volume_place,
                   pred_volume_place_weighted):
    ...
    # --- Prices ---
    electricity_prices = (
        gdf_full_updated[gdf_full_updated['charger_type'] == place_x_charger_type]
        .agg({'e_price': 'mean', 's_price': 'mean'})
        .to_dict()
    )
    ...
    place_x_performance = {
        **place_x_static,
        **electricity_prices,
        'charger_type': place_x_df['charger_type'][0]
    }
    place_x_performance_df = pd.DataFrame([place_x_performance])

    # --- Volume progression ---
    place_x_performance_df['volume_first_6'] = predicted_volume_place
    place_x_performance_df['volume_next_6'] = pred_volume_place_weighted
    place_x_performance_df['total_volume_first_year'] = (
        place_x_performance_df['volume_first_6'] + place_x_performance_df['volume_next_6']
    )
    place_x_performance_df['total_volume_after_first_year'] = pred_volume_place_weighted * 2
    place_x_performance_df['total_volume_2_years'] = (
        place_x_performance_df['total_volume_first_year'] +
        place_x_performance_df['total_volume_after_first_year']
    )

    # --- Profitability ---
    e_price = place_x_performance_df['e_price'][0]
    s_price = place_x_performance_df['s_price'][0]

    # Compute profits by period
    profit_6m_ramp = predicted_volume_place * s_price
    profit_6m_steady = pred_volume_place_weighted * s_price

    # Simulate period-by-period cumulative profit
    total_profit = 0
    months = 0

    for profit_6m in [profit_6m_ramp, profit_6m_steady] + [profit_6m_steady]*8:
        total_profit += profit_6m
        months += 6
        if total_profit >= total_investment_place_x:
            return f"{months} months to ROI"

    # If ROI not reached within simulated horizon, extrapolate linearly
    remaining = total_investment_place_x - total_profit
    extra_months = (remaining / profit_6m_steady) * 6
    return f"{months + extra_months:.0f} months to ROI"
